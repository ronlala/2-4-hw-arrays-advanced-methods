...
let startsLettera = groceryList.filter((item) => item.startsWith("A"));
  ...
  let endswithe = schoolList.filter((lastschool) => lastschool.includes("s"));
  ...
  // 11
  // Create a NEW array called "kidsDiscountAges" that contains the elements from the "patronAges" array that are 12 or under. T
  // he "patronAges" array should stay the same. Print "patronAges" and "kidsDiscountAges" to the console to verify your results.

  let kidsDiscountAges = patronAges.filter((twelveUnder) => twelveUnder < 12);
  console.log(patronAges);
  console.log(kidsDiscountAges);
  
  // 12
  // Find the first element from the "patronAges" array that are 12 or under.

  let firstElementUndertwelve = patronAges.find((twelveUnderfirst) => twelveUnderfirst < 12);


  
  // 13
  // Create a NEW array called "seniorsDiscountAges" that contains the elements from the "patronAges" array that are 60 or over. 
  // The "patronAges" array should stay the same. Print "patronAges" and "seniorsDiscountAges" to the console to verify your results.

  let seniorsDiscountAges = patronAges.filter((ageoversixty) => ageoversixty >= 60);
  
  // 14
  // Find the first element from the "patronAges" array that are 60 or over.
  let firstElementoversixty = patronAges.find((ageoversixty2) => ageoversixty2 >= 60);
  console.log(firstElementUndertwelve);

  // 15
  // Create a NEW array called "patronAgesIn2" that contains all the age elements from the patronAges array, but with 2 years added to each.
  //  The "patronAges" array should stay the same. Print patronAges and patronAgesIn2 to the console to verify your results.
 let patronAgesIn2 = patronAges.map((addtwoyears) => addtwoyears +2);
 console.log(patronAgesIn2);
 console.log(patronAges);

    // patronAges.forEach(add2yearstoages);
  // 16
  // Call a method on the patronAges array that will print: `patron age: ${age}` for each element in the array to the console. 
  // For example, the first item will be "patron age: 17" because 17 is the first element in patronAges. The "patronAges" array should not be affected. Print patronAges to be sure.
